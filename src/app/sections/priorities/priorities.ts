...
@Component({
  ...
  template: `
    <section id="priorities" class="shell py-10 md:py-14">
      ...
      <ul
        class="mt-10 md:mt-14 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 items-stretch list-none p-0"
      >
        @for (card of cards; track card.heading; let i = $index) {
          <li appReveal [appReveal]="i * 90" class="h-full">
            <article class="paper-card h-full p-6 md:p-7 flex flex-col">
              <app-check [size]="44" />
              <h3
                class="mt-4 font-display text-brand-blue text-xl md:text-2xl"
              >
                {{ card.heading }}
              </h3>
              <p class="mt-3 text-ink">{{ card.body }}</p>
            </article>
          </li>
        }
      </ul>
